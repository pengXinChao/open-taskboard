...
6. Verify the requested operation path. Add a comment with the changes, verification result, outcome, and remaining risks. Read the issue again, then move it to `in_review` with its current `version`. For issues whose `source` is `jira`, stop after the comment: do not move to `in_review`. Keep `in_progress` if work can continue; move to `blocked` only when a person must intervene.
